...
export const FinancialView: React.FC<FinancialViewProps> = ({
  ...
  const categoryData = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        fullName: "Public Health & Wellness",
        allocated: 1250,
        utilized: 980,
        rate: 78,
      },
  ...
  const rawOutliers: any[] = finData?.outlierProjects ||
    finData?.highValueProjects || [
      {
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 divide-y lg:divide-y-0 lg:divide-x divide-slate-800/80">
            ...
            <div className="mt-1.5 flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, Number(disbRate))}%` }}
                />
              </div>
              <span className="text-[10px] font-mono text-slate-400">
                ₹{(totalUtil / 10000000).toFixed(1)}Cr drawn
              </span>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-5">
            ...
            <div className="text-2xl sm:text-3xl font-bold font-mono tracking-tight text-red-400 tabular-nums">
              +{avgVariance}
              <span className="text-sm font-sans font-medium text-red-400">
                %
              </span>
            </div>
            <p className="mt-1 text-[11px] text-slate-400">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-800/70">
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Frequency distribution of sanctioned capital across scheme cost
                tiers. Red bar marks statutory outlier boundary.
              </p>
            </div>

            <div className="flex items-center gap-3 text-[10px] font-mono shrink-0">
              <span className="text-slate-400">
                Median: <strong className="text-slate-200">₹8.4L</strong>
              </span>
              <span className="text-slate-600">|</span>
              <span className="text-slate-400">
                P90: <strong className="text-slate-200">₹24.5L</strong>
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-800/60 text-[10px] text-slate-400">
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-sm bg-emerald-500" /> Standard
                Scale (&lt;₹30L)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-sm bg-amber-500" /> Elevated
                Budget (₹30L–₹50L)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-sm bg-red-500" /> Outlier Zone
                (&gt;₹50L)
              </span>
            </div>
            <span className="font-mono text-slate-400">
              Audit Rule AR-104 Active
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 border border-slate-800/90 rounded-lg bg-slate-900/50 p-5 space-y-4 flex flex-col justify-between">
          <div>
            <div className="pb-3 border-b border-slate-800/70">
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Comparison of sanctioned vs utilized funds across primary
                sectors (₹ Lakhs).
              </p>
            </div>
                ...
                <div key={idx} className="space-y-1">
                  ...
                  <div className="h-2 w-full bg-slate-800/80 rounded-full overflow-hidden flex">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, (cat.utilized / cat.allocated) * 100)}%`,
                      }}
                    />
                  </div>
          ...
          <div className="pt-3 border-t border-slate-800/60 flex items-center justify-between text-[10px] text-slate-400">
            <span className="flex items-center gap-1 text-slate-400">
              <Layers className="w-3 h-3 text-blue-400" />5 Scheme Categories
            </span>
      ...
      <div className="border border-slate-800/90 rounded-lg bg-slate-900/50 p-5 space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-800/70">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Works exhibiting severe budget divergence against identical
              category peer medians in the same district.
            </p>
          </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-800/60 text-[10px] text-slate-400">
          <span>
            Click any row to open the complete explainable AI audit trail and
            contractor breakdown.
          </span>
          <span className="font-mono text-slate-400">
            Showing {filteredOutliers.length} outlier cases
          </span>
        </div>
      </div>
    </motion.div>
